/**
 * 语法高亮类 将传入的字符按照指定的格式，格式化为高亮和缩进的HTML字符
 * @name Highlight
 * @grammar new ux.Highlight(code,syntax)
 * @param {String} code   需要格式化的代码字符串
 * @param {String} syntax 以何种语法高亮格式化，支持sql、c#、java、vbs、vb、js、html、xml、
 * @returns {ux.Highlight} ux.Highlight对象实例
...
function Highlight(code,syntax) 
{
    ...
    this.highlight    = function() {
        ...
        htmlTxt[htmlTxt.length] = ("<div style='width:640px;overflow:auto';height:auto>");
        //按分割字，分块显示
...
T.dom.ready(function(){
	...
	var es = T.dom.query('.CodeStyle');
	
	T.object.each(es,function(e){
		//var html = T.dom.getText(e);
		var html = e.innerHTML.replace(/<p>/ig,"").replace(/<\/p>/ig,"").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/<br>/ig,"\n");
		var type = T.dom.getAttr(e,'type') || 'html' ;
		//console.log(type)
		var code = new Highlight(html,type).highlight();
		e.innerHTML = code;
		//alert(code)
	})

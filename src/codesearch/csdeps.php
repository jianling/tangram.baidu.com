...
class Csdeps{
    ...
    function __construct($path){
        $this->dataDir="../../www/js/";
		$this->path = $path;
        $this->srcPath=realpath("tangram-code/");
        $this->source = $this->srcPath."/".$path;
		
		/*	遍历所有文件，但排除.svn|resource目录	*/
		$this->except = "/.*(\.svn|resource).*/";
	...
	private function get_base_functions() {
		...
		$allFiles =  $findFile->Find($this->source, array("except" => $this->except, "removePrefix" => $this->source));
	...
	public function walk_src($src) {
		...
		preg_match_all($this->except, join("\n", $allFiles), $matches);
		...
		foreach($allMatch[1] as $singleFile) {
			...
			if (isset($pkgMatch[1])) {
				...
				$package = str_replace("/", ".", str_replace("\\","/",$pkgMatch[1]) );
		...
		foreach($allMatch[1] as $singleFile) {
			...
			if (isset($funcMatch[1])) {
				$functionName = str_replace("/", ".", str_replace("\\", "/", $funcMatch[1]));
	...
	public function createDeps() {
		...
		$data = json_encode($this->walk_src($this->source));
		try {
